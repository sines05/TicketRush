...
func (h *OrderHandler) GetMyTickets(c *gin.Context) {
	...
	for _, t := range tickets {
		...
		zoneName := ""
		var price *float64
		if t.Seat.Zone.ID != uuid.Nil {
			zoneName = t.Seat.Zone.Name
			p := t.Seat.Zone.Price
			price = &p
			if t.Seat.Zone.Event.ID != uuid.Nil {
				eventTitle = t.Seat.Zone.Event.Title
				eventBannerURL = t.Seat.Zone.Event.BannerURL
			}
		}

		if eventTitle == "" {
			eventTitle = "TicketRush Event"
		}
		if zoneName == "" {
			zoneName = "Zone"
		}
