...
import { MessageCircleHeart, ShieldCheck, Sparkles, Star } from 'lucide-react';
...
function ReportCard({ report, index }) {
  ...
  return (
    <article className="tr-report-card relative w-[15.25rem] shrink-0 overflow-hidden rounded-2xl border border-white/70 bg-white/72 p-4 shadow-[0_18px_52px_-36px_rgba(6,95,105,.75)] backdrop-blur-xl transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_24px_62px_-38px_rgba(245,158,11,.65)] dark:border-white/10 dark:bg-white/[0.075] sm:w-[17rem]">
      <div className="absolute -right-8 -top-8 h-20 w-20 rounded-full bg-amber-300/24 blur-2xl dark:bg-amber-200/14" />
      <div className="absolute -bottom-10 left-8 h-20 w-24 rounded-full bg-cyan-300/20 blur-2xl dark:bg-cyan-300/10" />
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-2">
          ...
          <h2 className="text-2xl font-black uppercase tracking-tight text-foreground sm:text-3xl">
            From TicketRush with ❤️ 
          </h2>
        </div>
        
      </div>

      <div className="pointer-events-none absolute inset-y-16 left-0 z-10 w-20 bg-gradient-to-r from-background to-transparent sm:w-32" />
      <div className="pointer-events-none absolute inset-y-16 right-0 z-10 w-20 bg-gradient-to-l from-background to-transparent sm:w-32" />

      <div className="tr-report-rail relative -mx-4 overflow-hidden px-4 py-3 sm:-mx-8 sm:px-8">
        <div
